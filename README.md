...
Tavily 先以一个并发批次执行全部初始查询；只有未完成（未配置、不可用、空、超时、错误或 URL 无效）的查询才进入独立的 DDGS 批次，已由 Tavily 解决的查询不会重复请求 DDGS。DDGS 的阶段超时统一为 30 秒。一个查询的超时或错误不会丢弃其它已完成兄弟查询的结果；只要批次中至少有一个查询解决，就继续进入候选聚合与正文读取。正文抓取失败时，长度和 URL 安全条件合格、且经现有 Judge 判定直接相关的搜索摘要可以作为低置信度证据；这不会额外调用 LLM 复核，也不会绕过新鲜度、冲突或独立来源要求。
