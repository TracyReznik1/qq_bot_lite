...
项目刻意保持实用与清晰边界：
- **核心支持**：日常对话、按需自动或显式网页搜索、**网页 URL 前置直读与沙箱总结**、图片理解（多模态视觉输入）、对话历史上下文管理，以及细粒度会话 / 个人 / 全局结构化记忆；
- **安全防护**：内置三层 Prompt 注入防御（出站敏感凭据自动脱敏扫描、网页 XML 语义沙箱隔离、系统权威边界约束）；
- **不支持**：图片生成、图片编辑、主动发图、视频理解、天气或 B 站等外部专用工具插件、复杂 Agent。
- **说明**：`/search <关键词>` 专注于关键词证据化检索（`/search` 命令不提供独立 URL 直读）；当普通聊天消息中包含 HTTP/HTTPS 链接时，系统会自动进行网页前置安全直读并交由模型总结解答。
...
当用户在聊天中发送包含 HTTP/HTTPS 链接的消息时（例如 `https://example.com/article 请总结核心观点`），机器人会自动前置直读该网页：

1. **自动前置直读与搜索短路**：
   - 自动提取消息中的有效公网 URL（基于 RFC 3986 规范，遇到非 ASCII 中文字符或紧邻标点时精确截断，无需手动在链接后加空格）；
   - 执行带 SSRF 防护、重定向控制和 5 秒超时保护的正文抓取；
   - **抓取成功时**，直接将网页正文注入安全上下文供模型阅读回答，并**自动短路跳过**冗余在线搜索，大幅提升回复速度并节约搜索额度；
   - **抓取失败时**（如遇到 SPA 纯前端渲染无静态正文、强反爬拦截或超时），自动平滑降级为常规检索或常识对话，绝不中断会话。
2. **对话历史防膨胀**：
   - 持久化历史只记录用户原始输入文本（URL + 提问），不写入抓取的数千字网页正文，避免多轮会话导致上下文迅速耗尽。
3. **三层 Prompt 注入防御体系**：
   - **XML 语义沙箱隔离**：抓取的网页正文经 XML 字符实体全面转义，并置于 `<external_webpage_content>` 封闭标签内，杜绝网页文本伪造标签闭合越狱；
   - **系统级权威边界约束**：System Prompt 声明最高安全级，明确 `<external_webpage_content>` 为不可信第三方资料，严格禁止模型执行网页中的任何指令或角色扮演诱导；
   - **出站硬凭据脱敏拦截**：所有发往 QQ 的回复在经过 OneBot 传输前，统一通过出站敏感凭据扫描器。即便模型被极端越狱诱导输出 API Key（OpenAI `sk-`、Gemini `AIza...`、GitHub Token 等）、密码口令或私钥，扫描器会强制将其替换为 `[redacted:credential]`，牢牢守住凭据安全底线。
