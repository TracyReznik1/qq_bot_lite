...
**图片理解** — 可以直接发送图片，也可以发送图片加文字。每条消息最多 4 张图片，每张不超过 5 MiB，支持 JPEG、PNG、WebP 和 GIF；实际识别能力取决于当前配置的模型。

**三个常用命令**：

| 命令 | 说明 |
|---|---|
| `/search <关键词>` | 按关键词网页搜索（Tavily + DDGS） |
| `/remember <内容>` | 让 bot 记住你的偏好 |
| `/globalremember <内容>` | 全局记忆（仅管理员） |

还有 `/help`、`/reset`。

`/search <关键词>` 只进行关键词搜索；搜索服务可能在内部读取搜索结果页面以整理证据，但不提供独立 URL 直读。

**没有的功能** — 不提供图片生成、图片编辑、主动发图、视频理解、天气、B站和独立 URL 直读。普通聊天只配有搜索这一个工具。
