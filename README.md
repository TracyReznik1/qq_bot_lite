...
搜索按不可变预算分两档：`light`（1 查询 / 5 候选 / 2 正文读取 / 1 轮 / 8 秒，永不修复）、`standard`（1 个 direct 查询 + 至多 2 个补充查询 / 8 候选 / 5 正文读取 / 初始 + 至多 1 次修复 / 总语义查询不超过 4 / 20 秒）。多个初始并行查询只算一个初始轮次；只有 `standard` 最多再执行一次修复查询，修复后无条件停止，总检索轮次不超过两个。
