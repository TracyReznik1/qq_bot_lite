...
| `SEARCH_PLANNER_TIMEOUT` | 可选 | `8.0` | 搜索分析与规划阶段的超时秒数，至少按 `0.1` 处理。 |
...
搜索按不可变预算分两档：`light`（1 查询 / 5 候选 / 2 正文读取 / 1 轮，永不修复）、`standard`（1 个 direct 查询 + 至多 2 个补充查询 / 8 候选 / 5 正文读取 / 至多 1 次修复 / 总语义查询不超过 4 / 至多 2 轮）。Provider 回退不算语义查询、正文读取或轮次，不占用这些上限。

每个检索阶段使用自己的独立超时（分析/规划、Tavily 批次、DDGS 批次、Reader、Judge、Gap/Repair、Answer、Validator、Renderer），前一阶段耗时不扣减后一阶段的超时。请求分析仍只调用一次现有 LLM：事件时间或事实截止时间保留在搜索关键词中，只有用户明确询问某段时间发布的内容时才使用网页发布时间过滤。Tavily 会先移除相等的日期边界；服务拒绝有效范围参数时，会在同一阶段剩余预算内移除日期过滤后重试一次 Tavily，不把内部重试计为新的语义查询。

Tavily 先以一个并发批次执行全部初始查询；只有未完成（未配置、不可用、空、超时、错误或 URL 无效）的查询才进入独立的 DDGS 批次，已由 Tavily 解决的查询不会重复请求 DDGS。DDGS 的阶段超时默认为 15 秒。一个查询的超时或错误不会丢弃其它已完成兄弟查询的结果；只要批次中至少有一个查询解决，就继续进入候选聚合与正文读取。正文抓取失败时，长度和 URL 安全条件合格、且经现有 Judge 判定直接相关的搜索摘要可以作为低置信度证据；这不会额外调用 LLM 复核，也不会绕过新鲜度、冲突或独立来源要求。

总请求 watchdog 由各阶段预算之和派生：`light` 约 58 秒、`standard` 约 112 秒。这是防止非协作依赖无限运行的**安全上限，不是预期耗时**；阶段一旦完成自己的工作就会尽早停止，sufficient 证据会立即进入回答而无需耗尽上限。标准档至多执行一次修复查询，修复后无条件停止，不会出现第三轮。

搜索成功时，普通聊天和 `/search` 都直接输出自然语言答案，不会向 QQ 用户展示引用编号、来源标题或 URL；证据映射仅保留在后台校验与 Trace 中。不显示“搜索成功”或“检索完成”。“搜索结果可能不完整或不准确”的专业风险提示只用于医疗、法律、金融、安全等可能影响现实行动的高危请求；普通新闻、比赛、产品和技术搜索不显示该提示。搜索失败时会区分“在线搜索服务暂时不可用”“暂未找到足以确认结论的信息”和“名称或前提不一致”，且都不显示来源或 URL。

`/health` 返回 `search_ready` 和 `search_providers`（每个提供者的 `configured` / `available`），不暴露任何密钥、查询、URL 或证据正文。每个请求会记录一条 body-free 的 `SearchTrace`，分别统计路由、编排器启动、provider 尝试和证据充分性。
...
非 `/` 开头的文本进入普通聊天。事实型、混合型或歧义请求默认进入程序化的检索路由：无合法跳过理由时至少执行轻量搜索，聊天、创作、纯数学等明确无检索收益的任务才会跳过。搜索失败时按固定披露降级，不会无声明地切回普通模型回答；你也可以用显式命令强制搜索。

| 命令 | 别名 | 说明 |
|---|---|---|
| `/search <关键词>` | `/s <关键词>` | 显式网页搜索，强制走同一证据化检索管线，再由模型基于证据回答；URL 也只作为关键词搜索。 |
