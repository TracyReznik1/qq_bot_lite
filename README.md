...
| 参数 | 必需性 | 源码默认值 | 作用与配置方法 |
|---|---|---|---|
| `TAVILY_API_KEY` | 可选 | 空 | Tavily 是主搜索提供者；未配置 Tavily Key，或 Tavily 不可用、出错、超时、无结果、未产生有效 URL 时，使用 DDGS 回退。当两个提供者均失败、超时、无结果或不可用时，返回明确的在线检索失败披露（不同路径会说明在线检索未完成或无法完成在线核验），不会伪装成已经获得在线证据。 |
...
每个检索阶段使用自己的独立超时（分析/规划、Tavily 批次、DDGS 批次、Reader、Judge、Gap/Repair、Answer、Validator、Renderer），前一阶段耗时不扣减后一阶段的超时。Tavily 先以一个并发批次执行全部初始查询；只有未完成（未配置、不可用、空、超时、错误或 URL 无效）的查询才进入独立的 DDGS 批次，已由 Tavily 解决的查询不会重复请求 DDGS。DDGS 的阶段超时统一为 30 秒。一个查询的超时或错误不会丢弃其它已完成兄弟查询的结果；只要批次中至少有一个查询解决，就继续进入候选聚合与正文读取。
...
搜索成功时，普通聊天和 `/search` 都直接输出自然语言答案，不会向 QQ 用户展示引用编号、来源标题或 URL；证据映射仅保留在后台校验与 Trace 中。不显示“搜索成功”或“检索完成”。“搜索结果可能不完整或不准确”的专业风险提示只用于医疗、法律、金融、安全等可能影响现实行动的高危请求；普通新闻、比赛、产品和技术搜索不显示该提示。搜索失败时仍会明确披露“在线检索未完成”或“无法完成在线核验”。
