...
搜索按不可变预算分三档：`light`（1 查询 / 5 候选 / 2 正文读取 / 1 轮 / 8 秒）、`standard`（3 查询 / 8 候选 / 5 正文读取 / 初始 + 至多 1 修复 / 4 语义查询 / 20 秒）、`deep`（5 查询 / 15 候选 / 8 正文读取 / 初始 + 至多 1 修复 / 6 语义查询 / 40 秒）。多个初始并行查询只算一个初始轮次；`standard` 和 `deep` 最多再执行一个自适应修复查询，总检索轮次不超过两个。
