...
| 参数 | 必需性 | 源码默认值 | 作用与配置方法 |
|---|---|---|---|
| `TAVILY_API_KEY` | 可选 | 空 | DDGS 是主搜索提供者；DDGS 不可用、出错、超时或无结果时，若配置了 Tavily Key，则使用 Tavily 回退。当两个提供者均失败、超时、无结果或不可用时，返回明确的在线检索失败披露（不同路径会说明在线检索未完成或无法完成在线核验），不会伪装成已经获得在线证据。 |
...
每个检索阶段使用自己的独立超时（分析/规划、DDGS 批次、Tavily 批次、Reader、Judge、Gap/Repair、Answer、Validator、Renderer），前一阶段耗时不扣减后一阶段的超时。DDGS 先以一个并发批次执行全部初始查询；只有未完成（空/超时/错误/不可用/URL 无效）的查询才进入独立的 Tavily 批次，已由 DDGS 解决的查询不会重复请求 Tavily。一个查询的超时或错误不会丢弃其它已完成兄弟查询的结果；只要批次中至少有一个查询解决，就继续进入候选聚合与正文读取。

总请求 watchdog 由各阶段预算之和派生：`light` 约 34 秒、`standard` 约 65 秒。这是防止非协作依赖无限运行的**安全上限，不是预期耗时**；阶段一旦完成自己的工作就会尽早停止，sufficient 证据会立即进入回答而无需耗尽上限。标准档至多执行一次修复查询，修复后无条件停止，不会出现第三轮。
